...
def create_file(content="", title="Névtelen"):
...
def confirm_close():
    return messagebox.askyesno(
        message="A változásokat nem mentette, Biztos be akarja zárni a fület?",
        icon="question",
        title="Mentetlen változások vannak"
    )
...
def show_about_info():
    messagebox.showinfo(
        title="About",
        message="atty szövegszerkesztője"
    )

root = tk.Tk()
root.title("Szövegszerkesztő by atty")
...
file_menu.add_command(label="Új", command=create_file, accelerator="Ctrl+N")
file_menu.add_command(label="Megynitás", command=open_file, accelerator="Ctrl+O")
file_menu.add_command(label="Mentés", command=save_file, accelerator="Ctrl+S")
file_menu.add_command(label="Tab bezárása", command=close_current_tab, accelerator="Ctrl+Q")
file_menu.add_command(label="Kilépés", command=confirm_quit)
